        ...
        while dutyKey do
            if PlayerJob.name == jobRequired then
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent('KGCore:ToggleDuty')
                    dutyKey = false
                    break
                end
            else
                ...
                if dist < 25 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            TriggerServerEvent('kg-taxi:server:NpcPay', meterData.currentFare, NpcData.CrashCount == 0)
                            ...
                            KGCore.Functions.Notify(Lang:t('info.person_was_dropped_off'), 'success')
                            if NpcData.DeliveryBlip ~= nil then
                                RemoveBlip(NpcData.DeliveryBlip)
                            end
            ...
            if NpcData.NpcTaken then
                ...
                if vehicle and vehicle ~= 0 then
                    ...
                    if currentHealth < Config.Advanced.MinCabHealth then
                        TriggerEvent('kg-taxi:client:CancelTaxiNpc')
                        return KGCore.Functions.Notify(Lang:t('error.broken_taxi'), 'error')
                    end
                    ...
                    if lastVehicleHealth and currentHealth < lastVehicleHealth then
                        ...
                        if NpcData.CrashCount >= Config.Advanced.MaxCrashesAllowed then
                            TriggerEvent('kg-taxi:client:CancelTaxiNpc')
                            return KGCore.Functions.Notify(Lang:t('error.ride_canceled'), 'error')
                        end

                        local count = Config.Advanced.MaxCrashesAllowed - NpcData.CrashCount
                        KGCore.Functions.Notify(string.format(Lang:t('error.crash_warning'), count, count == 1 and Lang:t('error.time') or Lang:t('error.times')), 'error')
                    end
...
function TaxiGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            header = v.label,
            params = {
                event = 'kg-taxi:client:TakeVehicle',
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent('kg-taxi:client:TakeVehicle', function(data)
    ...
    if SpawnPoint then
        ...
        if CanSpawn then
            ...
            KGCore.Functions.Notify(Lang:t('info.no_spawn_point'), 'error')
        end
    else
        KGCore.Functions.Notify(Lang:t('info.no_spawn_point'), 'error')
        return
...
-- Events
RegisterNetEvent('kg-taxi:client:DoTaxiNpc', function()
    ...
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            KGCore.Functions.Notify(Lang:t('info.npc_on_gps'), 'success')
                        ...
                        if dist < 25 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    resetMeter()
                                    KGCore.Functions.Notify(Lang:t('info.go_to_location'))
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
            ...
            KGCore.Functions.Notify(Lang:t('error.already_mission'))
        end
    else
        KGCore.Functions.Notify(Lang:t('error.not_in_taxi'))
    end
end)

RegisterNetEvent('kg-taxi:client:CancelTaxiNpc', function()
...
RegisterNetEvent('kg-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            KGCore.Functions.Notify(Lang:t('error.missing_meter'), 'error')
        end
    else
...
RegisterNetEvent('kg-taxi:client:enableMeter', function()
    if meterIsOpen then
        SendNUIMessage({
            action = 'toggleMeter'
        })
    else
        KGCore.Functions.Notify(Lang:t('error.not_active_meter'), 'error')
    end
end)
...
RegisterNetEvent('kg-taxijob:client:requestcab', function()
    TaxiGarage()
end)
    ...
    while true do
        if not Config.UseTarget then
            ...
            if LocalPlayer.state.isLoggedIn then
                local Player = KGCore.Functions.GetPlayerData()
                            ...
                            if whitelistedVehicle() then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    if PlayerJob.onduty then
                                        ...
                                        KGCore.Functions.Notify('You need to be on duty')
                                    end
                                end
                            end
                        end
        ...
        exports['kg-target']:SpawnPed({
            ...
            target = {
                options = {
                    {
                        type = 'client',
                        event = 'kg-taxijob:client:requestcab',
    ...
    zone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsidePickupZone and not NpcData.NpcTaken then
                ...
                exports['kg-core']:DrawText(Lang:t('info.call_npc'), Config.DefaultTextLocation)
                callNpcPoly()
            end
        else
    ...
    deliveryZone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsideDropZone and NpcData.NpcTaken then
                ...
                exports['kg-core']:DrawText(Lang:t('info.drop_off_npc'), Config.DefaultTextLocation)
                dropNpcPoly()
            end
        else
...
function setupCabParkingLocation()
    ...
    taxiParking:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside and not Notified and Config.UseTarget then
            if whitelistedVehicle() then
                exports['kg-core']:DrawText(Lang:t('info.vehicle_parking'), Config.DefaultTextLocation)
                Notified = true
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if meterIsOpen then
                        TriggerEvent('kg-taxi:client:toggleMeter')
                        meterActive = false
                    end
                    TaskLeaveVehicle(PlayerPedId(), vehicle, 0)
                    Wait(2000) -- 2 second delay just to ensure the player is out of the vehicle
                    DeleteVehicle(vehicle)
                    KGCore.Functions.Notify(Lang:t('info.taxi_returned'), 'success')
                end
            end
        end
        Wait(1)
    ...
    dutyZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if not PlayerJob.onduty then
                exports['kg-core']:DrawText(Lang:t('info.on_duty'), 'left')
            else
                exports['kg-core']:DrawText(Lang:t('info.off_duty'), 'left')
            end
            onDuty()
        else
            dutyKey = false
